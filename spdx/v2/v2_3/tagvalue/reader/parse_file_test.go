...
func TestParserFileStartsNewFileAfterParsingFileNameTag(t *testing.T) {
	...
	parser := tvParser{
		...
		file: &spdx.File{FileName: fileOldName, FileSPDXIdentifier: "f1"},
	}
	fileOld := parser.file
...
func TestParserFileStartsNewPackageAfterParsingPackageNameTag(t *testing.T) {
	...
	parser := tvParser{
		doc:  &spdx.Document{Packages: []*spdx.Package{}},
		st:   psFile,
		pkg:  &spdx.Package{PackageName: p1Name, PackageSPDXIdentifier: "package1", Files: []*spdx.File{}},
		file: &spdx.File{FileName: f1Name, FileSPDXIdentifier: "f1"},
	}
	p1 := parser.pkg
	f1 := parser.file
...
func TestParserCanParseFileTags(t *testing.T) {
	...
	for algo, tc := range testChecksums {
		if err := parser.parsePairFromFile(
	...
	for _, attr := range attrs {
		err = parser.parsePairFromFile("FileAttributionText", attr)
		if err != nil {
			t.Errorf("expected nil error, got %v", err)
		}
	}
