...
func TestParserFailsParsingCreationInfoWithInvalidState(t *testing.T) {
	...
	err := parser.parsePairFromCreationInfo("SPDXVersion", spdx.Version)
	if err == nil {
...
func TestParserCanParseCreationInfoTags(t *testing.T) {
	...
	err := parser.parsePairFromCreationInfo("LicenseListVersion", "1.3")
	if err != nil {
		t.Errorf("expected nil error, got %v", err)
	}
	if parser.doc.CreationInfo.LicenseListVersion != "1.3" {
...
func TestParserCIMovesToOtherLicenseAfterParsingDocument(t *testing.T) {
	parser := tvParser{
		doc: &spdx.Document{},
		st:  psCreationInfo,
	}
